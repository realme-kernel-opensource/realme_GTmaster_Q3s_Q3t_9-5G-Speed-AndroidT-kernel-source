...
static int st54spi_parse_dt(struct device *dev, struct st54spi_data *pdata)
{
    ...
    pdata->pctrl_mode_spi = pinctrl_lookup_state(pdata->pctrl, "qupv3_se12_spi_active");
    if (IS_ERR(pdata->pctrl_mode_spi)) {
        dev_err(dev, "%s: Unable to find qupv3_se12_spi_active: %d\n",
            __FILE__, PTR_ERR(pdata->pctrl_mode_spi));
        return -1;
    }

    pdata->pctrl_mode_idle = pinctrl_lookup_state(pdata->pctrl, "qupv3_se12_spi_sleep");
    if (IS_ERR(pdata->pctrl_mode_idle)) {
        dev_err(dev, "%s: Unable to find qupv3_se12_spi_sleep: %d\n",
