...
static int oplus_pps_get_status(struct oplus_pps_chip *chip)
{
	...
	if (chip->oplus_pps_status == OPLUS_10V_30W_STATUS) {
		...
		chip->charger_output_current = chip->ask_charger_current;
	} else {
	...
	case PPS_BAT_TEMP_NATURAL:
		if (vbat_temp_cur > chip->pps_strategy1_batt_high_temp0) {
			...
			if (chip->pps_batt_over_low_temp != -EINVAL) {
				ret = 2;
				chg_err("for cell balancing, the ret:%d\r\n", ret);
			}
		}
		break;
...
static void oplus_pps_status_check(struct oplus_pps_chip *chip)
{
	...
	if (chip->ap_batt_volt > BAT_FULL_1TIME_TH_PPS) {
		chip->fg_batt_full = 1;
	}
	...
	if (chip->pps_status > OPLUS_PPS_STATUS_OPEN_OVP_CP) {
...
/*
		if ((chip->ap_input_volt - chip->target_charger_volt) > 1000
				|| (chip->target_charger_volt - chip->ap_input_volt) > 1000) {
			update_size = 500;
		} else {
			update_size = 100;
		}
...
*/
	...
	if (chip->ask_charger_volt < chip->target_charger_volt) {
		...
		chg_debug("pps chargering volt okay 11\n");
	...
	case OPLUS_PPS_STATUS_CUR_INCREASE:
		...
		chip->target_charger_current = oplus_pps_get_target_current(chip);
 		...
 		//if ((chip->ap_input_current - chip->target_charger_current) > 1000 || (chip->target_charger_current - chip->ap_input_current) > 1000) {
 		if ((chip->ask_charger_current - chip->target_charger_current) > 1000 || (chip->target_charger_current - chip->ask_charger_current) > 1000) {
			update_size = 500;
		} else {
			update_size = 50;
		}
		...
		if (chip->ask_charger_current < (chip->target_charger_current - 500)) {
			...
			if (chip->ask_charger_current < chip->target_charger_current - 50) {
				curr_increase_cnt++;
