...
extern void set_ux_task_to_prefer_cpu_mtk(struct task_struct *task, int *orig_target_cpu, bool *cond);
...
extern unsigned long oplus_task_util(struct task_struct *p);
	...
	switch (scene) {
		...
		case SA_SLIDE:
			return sysctl_slide_boost_enabled
				|| sysctl_sched_assist_scene & SA_SLIDE;
		case SA_INPUT:
