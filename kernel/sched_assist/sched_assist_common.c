...
OUT:
	if (param_ux_debug == UX_DEBUG_LEVEL_HEAVY)
		sched_assist_systrace((*p)->ux_state, "ux_state_%d", rq->cpu);
...
#ifdef CONFIG_SCHED_WALT
	if (test_task_identify_ux(task, SA_TYPE_ID_CAMERA_PROVIDER)) {
		boost = is_task_util_over(task, THRESHOLD_BOOST);
	}
...
void sched_assist_target_comm(struct task_struct *task)
{
	...
	if (strstr(grp_leader->comm, CAMERA_MAINTHREAD_NAME) && (strstr(task->comm, CAMERA_PREMR_NAME)
...
//#ifdef CONFIG_UXCHAIN_V2
	if (se_task->ux_once) {
		...
		se_task->ux_once = 0;
		sched_assist_systrace(se_task->pid, "ux_biglock");
		return;
	}
#endif
...
static inline bool is_ux_task_prefer_cpu(struct task_struct *task, unsigned int cpu)
{
	struct ux_sched_cputopo ux_cputopo = ux_sched_cputopo;
	int cls_nr = ux_cputopo.cls_nr - 1;
...
unsigned long oplus_task_util(struct task_struct *p)
{
#ifdef CONFIG_OPLUS_SYSTEM_KERNEL_QCOM
	return task_util(p);

#else /* NOT CONFIG_OPLUS_SYSTEM_KERNEL_QCOM */
#ifdef CONFIG_SCHED_WALT
...
bool is_task_util_over(struct task_struct *task, int threshold)
{
	...
	demand_over = oplus_task_util(task) >= threshold;
...
static inline int get_task_cls_for_scene(struct task_struct *task)
{
	...
	if (sched_assist_scene(SA_LAUNCHER_SI) && is_task_util_over(task, sysctl_boost_task_threshold)) {
		return cls_mid;
	}
...
void set_ux_task_to_prefer_cpu_mtk(struct task_struct *task, int *orig_target_cpu, bool *cond) {
	*cond = true;
	set_ux_task_to_prefer_cpu(task, orig_target_cpu);
}
...
void set_ux_task_to_prefer_cpu(struct task_struct *task, int *orig_target_cpu) {
	...
	struct ux_sched_cputopo ux_cputopo = ux_sched_cputopo;
	int cls_nr = ux_cputopo.cls_nr - 1;
	int cpu = *orig_target_cpu;
	int direction = -1;
	int ux_select = 0; /* 0: defalt, 1: not found suitable cpu, 2: found */
...
OUT:
	if (param_ux_debug == UX_DEBUG_LEVEL_HEAVY) {
		if (ux_select == 1) {
			sched_assist_systrace(cpu_isolated_mask->bits[0], "ux_cpu_isolated");
			sched_assist_systrace(cpu_online_mask->bits[0], "ux_cpu_online");
		}
		sched_assist_systrace(task->pid * 100 + *orig_target_cpu, "ux_cpu_%d", ux_select);
	}
		...
		size_t count, loff_t *ppos)
{
	...
	len = snprintf(buffer, sizeof(buffer), "ux_state=%d inherit=%llx(bi:%d fu:%d rw:%d mu:%d)\n",
		ux_state, task->inherit_ux,
		test_inherit_ux(task, INHERIT_UX_BINDER), test_inherit_ux(task, INHERIT_UX_FUTEX),
		test_inherit_ux(task, INHERIT_UX_RWSEM), test_inherit_ux(task, INHERIT_UX_MUTEX));
	...
	void __user *buffer, size_t *lenp, loff_t *ppos)
{
	...
	if ((save_sa & SA_SLIDE) && (save_sa & SA_INPUT)) {
		save_sa &= ~SA_INPUT;
...
{
	int set_ux_once = (sem == &(current->mm->mmap_sem));
...
{
	if (sem == &(current->mm->mmap_sem) && sysctl_uxchain_v2) {
