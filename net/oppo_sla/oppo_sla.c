		...
		if(!game_uid[index].switch_time ||
			...
			**avoid game app ping through wifi.**/
			...
			oppo_sla_send_to_user(SLA_SWITCH_GAME_NETWORK, (char *)game_bp_info,
				sizeof(game_bp_info));
			printk("oppo_sla_game_rtt:uid = %u,game rtt = %u,wlan_bad = %d,game_rx_bad = %d,oppo_game_latency_info=%d,changing to cellular...\n",
	...
	if(!wlan_bad &&
	   ...
	   (game_rx_bad || oppo_game_latency_info >= max_latency)) {
		...
		if (!game_uid[index].switch_time ||
			...
			oppo_sla_send_to_user(SLA_SWITCH_GAME_NETWORK, (char *)game_bp_info,
				sizeof(game_bp_info));
			printk("oppo_sla_game_rtt:uid = %u,game rtt = %u,wlan_bad = %d,game_rx_bad = %d,oppo_game_latency_info=%d,changing to wlan...\n",
				 uid, game_rtt, wlan_bad, game_rx_bad, oppo_game_latency_info);
			/* SuPei@CONNECTIVITY.WIFI.NETWORK.SLA.1379767 2021/04/25, Add for game latency */
			oppo_game_latency_info = 0;
			/* OPLUS_SLA_GAME_LATENCY */
...
static int sla_skb_reroute(struct sock *sk, struct sk_buff *skb, struct nf_conn *ct,
	const struct nf_hook_state *state)
{
	int err;

	err = ip_route_me_harder(state->net, sk, skb, RTN_UNSPEC);
...
sla_reroute:
	ret = sla_skb_reroute(sk, skb, ct, state);
	return ret;
