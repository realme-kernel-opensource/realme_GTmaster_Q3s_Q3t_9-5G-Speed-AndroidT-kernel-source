	...
	char *buf)
{
	...
	snprintf(tb, 1024,
		 "sia81xx_cmd_show : rst pin status : %d, chip id = 0x%02x \r\n"
	...
	size_t len)
{
	...
	case SIA81XX_CMD_SET_OWI_DELAY : {
		...
		if (temp_us <
				MAX_OWI_PULSE_GAP_TIME_US) /* only for test, pulse width must be < 1ms */
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("[debug][%s] %s: ucontrol = %ld", LOG_FLAG, __func__,
		ucontrol->value.integer.value[0]);
...
static int sia81xx_probe(struct platform_device *pdev)
{
	...
	pr_info("[ info][%s]  %s: line = %d  sia81xx_sia_boost_vol = %d\r\n", LOG_FLAG,
		__func__, __LINE__, sia_boost_vol);
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4,16,28))
	ret = snd_soc_register_component(&pdev->dev, &soc_component_dev_sia81xx, NULL,
					 0);
#else
