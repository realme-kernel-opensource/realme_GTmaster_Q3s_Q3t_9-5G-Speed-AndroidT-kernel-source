...
int tfa_irq_clear(struct tfa_device *tfa, enum tfa9912_irq bit)
{
	...
	if (bit == tfa9912_irq_all) {
		/* operate on all bits */
		for (reg = TFA98XX_INTERRUPT_IN_REG1; reg < TFA98XX_INTERRUPT_IN_REG1 + 3;
				reg++)
...
int tfa_irq_ena(struct tfa_device *tfa, enum tfa9912_irq bit, int state)
{
	...
	if (bit == tfa9912_irq_all) {
		...
		for (reg = TFA98XX_INTERRUPT_ENABLE_REG1;
			...
			tfa->interrupt_enable[reg - TFA98XX_INTERRUPT_ENABLE_REG1] = state ? 0xffff :
					0; /* all bits */
		}
		...
		unsigned char *bytes)
{
	...
	while (num_bytes > 0) {
		...
		error = tfa98xx_read_data_v6(tfa, FAM_TFA98XX_CF_MEM, burst_size,
					     bytes + offset);
...
enum Tfa98xx_Error
tfa98xx_dsp_write_mem_word_v6(struct tfa_device *tfa, unsigned short address,
			      int value, int memtype) {
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
enum Tfa98xx_Error
tfa98xx_dsp_support_framework_v6(struct tfa_device *tfa,
