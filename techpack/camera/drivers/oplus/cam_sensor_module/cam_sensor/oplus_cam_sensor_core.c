...
#define S5KGM1ST_EEPROM_MODULE_ID_ADDR 0x0000
#define S5KGM1ST_EEPROM_SID (0xA0 >> 1)
#define S5KGM1ST_FIRST_SENSOR_ID 0x08D1
#define S5KGM1ST_FIRST_MODULE_ID 0x05
#define S5KGM1ST_SECOND_SENSOR_ID 0x08D2
#define S5KGM1ST_SECOND_MODULE_ID 0x02
...
uint32_t cam_override_chipid(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	} else if (slave_info->sensor_id == S5KGM1ST_FIRST_SENSOR_ID ||
		slave_info->sensor_id == S5KGM1ST_SECOND_SENSOR_ID) {
		memcpy(&ee_cci_client, s_ctrl->io_master_info.cci_client,
				sizeof(struct cam_sensor_cci_client));
		 ee_cci_client.sid = S5KGM1ST_EEPROM_SID;

		rc = cam_cci_i2c_read(&ee_cci_client,
			S5KGM1ST_EEPROM_MODULE_ID_ADDR,
			&ee_moduleid, CAMERA_SENSOR_I2C_TYPE_WORD,
			CAMERA_SENSOR_I2C_TYPE_BYTE);

		CAM_INFO(CAM_SENSOR, "distinguish s5kgm1st camera module, moudle id : 0x%x , sensor id : 0x%x", ee_moduleid, slave_info->sensor_id);

		if (ee_moduleid == S5KGM1ST_FIRST_MODULE_ID) {
			chipid = S5KGM1ST_FIRST_SENSOR_ID;
		} else if (ee_moduleid == S5KGM1ST_SECOND_MODULE_ID) {
			chipid = S5KGM1ST_SECOND_SENSOR_ID;
		} else {
			chipid = S5KGM1ST_FIRST_SENSOR_ID;
		}
	} else {
